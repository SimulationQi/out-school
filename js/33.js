...
window.onload = function(){
  ...
  let componentStyleHtml = '.component_mask{' +
    ...
    '.component_toast {' +
    'position: relative;top: 80%;margin: 0 auto;min-width: 200px;width: 50%;max-width: 300px;' +
    'padding: 2px;background: #fff;text-align: center;border-radius: 5px;}' +
    '.component_dialog {' +
...
ToastClass.prototype.show = function (options) {
  ...
  innerHTMLArr.push('<div style="font-size: 13px;margin:10px 0;">' + text + '</div></div></div>');
  ...
  this.clearTimeoutObj=null;
  
  if (duration) {
    this.clearTimeoutObj=setTimeout (function () {
